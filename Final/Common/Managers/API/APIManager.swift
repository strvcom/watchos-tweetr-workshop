...
final class APIManager: APIManaging {
    ...
    func request(_ endpoint: Router) async throws -> Data {
        ...
        if
            let authorizableEndpoint = endpoint as? AccessTokenAuthorizable,
            let header = try authorizableEndpoint.getAuthenticationHeader(token: token, tokenSecret: tokenSecret)
        {
